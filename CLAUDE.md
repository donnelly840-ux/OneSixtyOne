...
The pillars and capabilities scrubs are vanilla JS, not GSAP, so they don't get `ScrollTrigger.config({ ignoreMobileResize: true })`. Both were choppy on phones for the same reasons (fixed Sep 2026):
