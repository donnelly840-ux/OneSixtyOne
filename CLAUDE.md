...
**Touch devices: GSAP owns scrolling (`ScrollTrigger.normalizeScroll`, Sep 2026).** The CSS scroll-driven path below still jumped on the user's iPhone. Root cause is timing, not painting: iOS scrolls the page on its own process while the capabilities words are positioned from an older scroll value, so they step unevenly (converting them to SVG would not help — it changes how they're drawn, not when they move). On `(pointer: coarse)` only, the GSAP block calls `ScrollTrigger.normalizeScroll({ allowNestedScroll: true })`, which moves scrolling onto the JS thread so the page and the words land in the same frame, then sets `window.__osoTicker = true` and runs every function in `window.__osoTickFns` from `gsap.ticker` — i.e. right after GSAP has moved the page. The pillars and capabilities scripts register there and stop scheduling their own rAF loops once the ticker is on (their own rAF could run before GSAP's and read last frame's scroll). Desktop scrolling stays native. **Watch for:** changed momentum feel on phones (it's emulated), and nested scroll in the privacy/contact modals (`allowNestedScroll` should cover it). If this is ever removed, remove the ticker hand-off with it.
